class Anthill < GameObject
  ...
  def draw
    ...
    @last_update = Time.now
    puts "#{'Anthill'.colorize(color: :light_cyan)} ==> all space: #{@all_space.to_s.light_green}; taken space: #{@taken_space.to_s.light_green}; food stock: #{@food_stock.to_s.light_green}; larvas: #{@larvas_number.to_s.light_green}; workers: #{@workers_number.to_s.light_green}; providers: #{@providers_number.to_s.light_green}; pop: #{@object_pool.size.to_s.light_green}; #{'Queen'.colorize(color: :light_cyan)} ==> live level: #{@object_pool.queen.live_level.to_s.light_green}; food level: #{@object_pool.queen.food_level.to_s.light_green}"
  end
